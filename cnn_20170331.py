...
def cell_extract(form):
	...
	lines = cv2.HoughLines(thr, 1, (np.pi/180), 1000)
	y1_list = list();
	x1_list = list();
	...
	for y in range(0,length_y):
		for x in range(0, length_x+1):
			cells.append(form[y1_list[y]:y1_list[y]+y_gap,x1_list[x]:x1_list[x]+x_gap])
...
def num_segment(img):
	...
	for rect in rects:
		if ((rect[2]+rect[3])>20):
			...
			number_seq.append([roi_black, pt_col, pt_row])	# number_seq is a 2D list
			def getKey(item):
				return item[1]
			number_seq = sorted(number_seq, key = getKey)

	return number_seq

def num_recognition(number_seq):
	num_seg_recog = []

	for n in range(0, len(number_seq)):
		single_num = number_seq[n][0].reshape(1, 28, 28, 1)
		...
		num_seg_recog.append(nbr[0])

	return num_seg_recog
